...
- **macOS self-update signature caveat (verify on next release)**: `ApplyUpdate` (`update_unix.go`) swaps the inner binary inside the `.app` bundle. `scripts/build-macos.sh` now **strips** the ad-hoc signature (SVC's MDM approach) instead of ad-hoc-signing, so the bundle is unsigned — swapping the inner binary doesn't break a seal (there's none). The arm64 kernel re-applies an ad-hoc signature to the inner binary at exec time. This path has NOT been exercised end-to-end yet (no v2.0.0 has shipped). If a self-updated macOS build fails to relaunch, re-run the DMG installer or `scripts/run-first-installed-local-mac.sh`.
